...
import { verifyProviderConfig } from "./providerModel.js";
...
function ProviderForm({ onConnected, providerType }: ProviderFormProps) {
  ...
  const verifyAndSave = async (nextValues: FormValues): Promise<void> => {
    ...
    try {
      ...
      setError(formatErrorMessage(caughtError, config.apiKey));
      ...
      <Box flexDirection="column">
        ...
        {FORM_FIELDS.map((field, index) => (
          ...
          >
            ...
            {field.label}: {formatFieldValue(field.id, values[field.id])}
...
function formatFieldValue(fieldId: FieldId, value: string): string {
  if (fieldId !== "apiKey") {
    return value;
  }
...
function formatErrorMessage(error: unknown, apiKey: string): string {
